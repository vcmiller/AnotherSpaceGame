...
namespace SBR.Editor {
    ...
    public class BrushInspector : UnityEditor.Editor {
        ...
        private float scaleMvtX;
        private float scaleMvtY;
        private float scaleMvtZ;
        ...
        private void UpdateFreeform() {
            ...
            if (selectedFaces.Count > 0 || selectedEdges.Count > 0 || selectedVertices.Count > 0) {
                ...
                if (Tools.current == Tool.Move) {
                    ...
                    Handles.DrawLine(vertexCenter, vertexCenter + normalAxis * hs);
                    MoveHandle(vertices, vertexCenter, normalAxis, ref moved, hs);

                    Handles.color = Handles.xAxisColor;
                    Handles.DrawLine(vertexCenter, vertexCenter + rightAxis * hs);
                    MoveHandle(vertices, vertexCenter, rightAxis, ref moved, hs);

                    Handles.color = Handles.yAxisColor;
                    Handles.DrawLine(vertexCenter, vertexCenter + upAxis * hs);
                    MoveHandle(vertices, vertexCenter, upAxis, ref moved, hs);
                } else if (Tools.current == Tool.Scale) {
                    Handles.color = Handles.zAxisColor;
                    Handles.DrawLine(vertexCenter, vertexCenter + normalAxis * (hs + scaleMvtZ));
                    ScaleHandle(vertices, vertexCenter + normalAxis * scaleMvtZ, normalAxis, ref moved, ref scaleMvtZ, hs);

                    Handles.color = Handles.xAxisColor;
                    Handles.DrawLine(vertexCenter, vertexCenter + rightAxis * (hs + scaleMvtX));
                    ScaleHandle(vertices, vertexCenter + rightAxis * scaleMvtX, rightAxis, ref moved, ref scaleMvtX, hs);

                    Handles.color = Handles.yAxisColor;
                    Handles.DrawLine(vertexCenter, vertexCenter + upAxis * (hs + scaleMvtY));
                    ScaleHandle(vertices, vertexCenter + upAxis * scaleMvtY, upAxis, ref moved, ref scaleMvtY, hs);
                }
            }
